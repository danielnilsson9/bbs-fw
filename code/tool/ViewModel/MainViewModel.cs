...
public class MainViewModel : ObservableObject
{
    ...
    {
        get {
            return new DelegateCommand(OnOpenConfig);
        }
    }
    ...
    {
        get {
            return new DelegateCommand(OnSaveConfig);
        }
    }
    ...
    {
        get {
            return new DelegateCommand(OnSaveLog);
        }
    }
    ...
    {
        get {
            return new DelegateCommand(OnReadFlash);
        }
    }
    ...
    {
        get {
            return new DelegateCommand(OnWriteFlash);
        }
    }
    ...
    {
        get {
            return new DelegateCommand(OnResetFlash);
        }
    }
    ...
    {
        get {
            return new DelegateCommand(OnExit);
        }
    }
    ...
    {
        get {
            return new DelegateCommand(OnShowAbout);
        }
    }
    ...
    private async void OnReadFlash()
    {
        ...
        {
            MessageBox.Show("Failed to read configuration from flash, timeout occured.", "Error", MessageBoxButton.OK,
    ...
    private async void OnWriteFlash()
    {
        ...
        if (!res.Timeout)
        {
            ...
            {
                MessageBox.Show("Failed to write configuration to flash, try again.", "Error", MessageBoxButton.OK,
                                MessageBoxImage.Error);
            }
        }
        else
        {
            MessageBox.Show("Failed to write configuration to flash, timeout occured.", "Error", MessageBoxButton.OK,
                            MessageBoxImage.Error);
        }
    }
    ...
    private async void OnResetFlash()
    {
        ...
        if (!res.Timeout)
        {
            ...
            {
                MessageBox.Show("Failed to reset configuration, try again.", "Error", MessageBoxButton.OK,
                                MessageBoxImage.Error);
            }
        }
        else
        {
            MessageBox.Show("Failed to reset configuration, timeout occured.", "Error", MessageBoxButton.OK,
                            MessageBoxImage.Error);
        }
    }
    ...
    private void OnShowAbout()
    {
        ...
        MessageBox.Show($"Version: {version.Major}.{version.Minor}.{version.Build}\nAuthor: Daniel Nilsson",
                        "BBS-FW Tool", MessageBoxButton.OK, MessageBoxImage.Information);
    }
    ...
    {
        if (ConnectionVm.ConfigVersion < Configuration.MinVersion ||
            ConnectionVm.ConfigVersion > Configuration.MaxVersion)
        {
            MessageBox.Show("Unsupported firmware config version. Please use BBS-FW Config Tool for firmware version " +
                                ConnectionVm.FirmwareVersion + " to read configuration from flash.",
    ...
    private bool VerifyConfigVersionForWrite()
    {
        ...
        {
            MessageBox.Show("Unsupported firmware config version. Please use BBS-FW Config Tool for firmware version " +
                                ConnectionVm.FirmwareVersion +
                                " in order to write configuration to flash, or upgrade firmware to latest version.",
                            "Error", MessageBoxButton.OK, MessageBoxImage.Error);
            return false;
        }
