...
public class CalibrationViewModel : ObservableObject
{
    ...
    {
        get {
            return new DelegateCommand(OnSaveVoltageCalibration);
        }
    }
    ...
    {
        get {
            return new DelegateCommand(OnResetVoltageCalibration);
        }
    }
    ...
    private async void OnSaveVoltageCalibration()
    {
        ...
        {
            MessageBox.Show("Measured Battery Voltage must be in range [1, 100]", "Error", MessageBoxButton.OK,
                            MessageBoxImage.Error);
            return;
        }

        var res =
            await _connectionVm.GetConnection().CalibrateBatteryVoltage(MeasuredBatteryVolts, TimeSpan.FromSeconds(3));
        if (!res.Timeout)
        {
            if (res.Result)
            {
                MessageBox.Show("Voltage calibration saved!", "Success", MessageBoxButton.OK,
                                MessageBoxImage.Information);
            }
            else
            {
                MessageBox.Show("Failed to save voltage calibration, check log.", "Error", MessageBoxButton.OK,
                                MessageBoxImage.Error);
            }
        }
        else
        {
            MessageBox.Show("Failed to save voltage calibration, timeout occured.", "Error", MessageBoxButton.OK,
                            MessageBoxImage.Error);
        }
    }
    ...
    private async void OnResetVoltageCalibration()
    {
        ...
        if (!res.Timeout)
        {
            if (res.Result)
            {
                MessageBox.Show("Voltage calibration reset!", "Success", MessageBoxButton.OK,
                                MessageBoxImage.Information);
            }
            else
            {
                MessageBox.Show("Failed to reset voltage calibration, check log.", "Error", MessageBoxButton.OK,
                                MessageBoxImage.Error);
            }
        }
        else
        {
            MessageBox.Show("Failed to reset voltage calibration, timeout occured.", "Error", MessageBoxButton.OK,
                            MessageBoxImage.Error);
        }
    }
}
