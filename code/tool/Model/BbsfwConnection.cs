...
public class BbsfwConnection
{
    ...
    private void ProcessInputBuffer()
    {
        lock (_rxBuffer)
        {
            while (true)
            {
                ...
                if (result == Discard)
                {
                    System.Diagnostics.Debug.WriteLine("Discarding: " +
                                                       BitConverter.ToString(_rxBuffer.ToArray()).Replace("-", " "));
                    _rxBuffer.Clear();
                }
                else if (result > 0)
                {
    ...
    private int ProcessReadResponseConfig()
    {
        ...
        if (_rxBuffer.Count > 3)
        {
            ...
            if (version < Configuration.MinVersion || version > Configuration.MaxVersion ||
                size != Configuration.GetByteSize(version))
            {
                System.Diagnostics.Debug.WriteLine(
                    "Config read from flash is of an unsupported version or is corrupt, discarding.");
                return Discard;
            }
        }
        else
