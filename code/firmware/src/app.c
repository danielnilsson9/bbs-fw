...
void app_init()
{
    ...
    uint16_t padded_voltage_range_x100 =
        (uint16_t)(full_voltage_range_x100 * (100 - BATTERY_FULL_OFFSET_PERCENT - BATTERY_EMPTY_OFFSET_PERCENT) / 100);

    lvc_ramp_down_end_voltage_x100 =
        (uint16_t)(lvc_voltage_x100 + (full_voltage_range_x100 * BATTERY_EMPTY_OFFSET_PERCENT / 100));
    lvc_ramp_down_start_voltage_x100 =
        (uint16_t)(lvc_ramp_down_end_voltage_x100 + ((padded_voltage_range_x100 * LVC_RAMP_DOWN_OFFSET_PERCENT) / 100));
...
void app_process()
{
    ...
    else
    {
        ...
        if (throttle_override && (assist_level_data.level.flags & ASSIST_FLAG_PAS) &&
...
void app_set_wheel_max_speed_rpm(uint16_t value)
{
    if (global_speed_limit_rpm != value)
    {
        ...
        global_throttle_speed_limit_rpm_x10 =
            ((int32_t)global_speed_limit_rpm * g_config.throttle_global_spd_lim_percent) / 10;
...
void apply_pas_cadence(uint8_t *target_current, uint8_t throttle_percent)
{
    if ((assist_level_data.level.flags & ASSIST_FLAG_PAS) && !(assist_level_data.level.flags & ASSIST_FLAG_PAS_TORQUE))
    {
        if (pas_is_pedaling_forwards() && pas_get_pulse_counter() > g_config.pas_start_delay_pulses)
        {
            ...
            {
                uint8_t current =
                    (uint8_t)MAP16(throttle_percent, 0, 100, 0, assist_level_data.level.target_current_percent);
            ...
            else
            {
                ...
                if (g_config.pas_keep_current_percent < 100)
                {
                    ...
                    {
                        uint32_t cadence =
                            MIN(pas_get_cadence_rpm_x10(), assist_level_data.keep_current_ramp_end_rpm_x10);

                        // ramp down current towards keep_current_target_percent with rpm above
                        // keep_current_ramp_start_rpm_x10
                        *target_current = MAP32(cadence,                                           // in
...
void apply_pas_torque(uint8_t *target_current)
{
    if ((assist_level_data.level.flags & ASSIST_FLAG_PAS) && (assist_level_data.level.flags & ASSIST_FLAG_PAS_TORQUE))
    {
        if (pas_is_pedaling_forwards() &&
            (pas_get_pulse_counter() > g_config.pas_start_delay_pulses || speed_sensor_is_moving()))
        {
            ...
            uint16_t target_current_amp_x100 = (uint16_t)(((uint32_t)10 * pedal_power_w_x10 *
                                                           assist_level_data.level.torque_amplification_factor_x10) /
                                                          battery_voltage_x10);
...
void apply_cruise(uint8_t *target_current, uint8_t throttle_percent)
{
    ...
    if ((assist_level_data.level.flags & ASSIST_FLAG_CRUISE) && throttle_ok())
    {
        ...
        else if (cruise_paused && !cruise_block_throttle_return && throttle_percent > 50 &&
                 pas_is_pedaling_forwards() && pas_get_pulse_counter() > CRUISE_ENGAGE_PAS_PULSES)
        {
            cruise_paused = false;
            cruise_block_throttle_return = true;
        }

        // reset flag tracking throttle to make sure throttle returns to idle position before engage/disenage cruise
        // with throttle touch
...
bool apply_throttle(uint8_t *target_current, uint8_t throttle_percent)
{
    ...
    {
        uint8_t current = (uint8_t)MAP16(throttle_percent, 0, 100, g_config.throttle_start_percent,
                                         assist_level_data.level.max_throttle_current_percent);
...
bool apply_speed_limit(uint8_t *target_current, uint8_t throttle_percent, bool pas_engaged, bool throttle_override)
{
    ...
    bool global_throttle_limit_active =
        !pas_engaged && throttle_percent > 0 && g_config.throttle_global_spd_lim_percent > 0 &&
    ...
    if (max_speed_rpm_x10 > 0)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                uint8_t tmp = (uint8_t)MAP32(current_speed_rpm_x10, max_speed_ramp_low_rpm_x10,
                                             max_speed_ramp_high_rpm_x10, *target_current, 1);
...
bool apply_thermal_limit(uint8_t *target_current)
{
    ...
    if (max_temp >= (MAX_TEMPERATURE - MAX_TEMPERATURE_RAMP_DOWN_INTERVAL))
    {
        ...
        uint8_t tmp = (uint8_t)MAP32(max_temp_x100,                                                // value
...
bool apply_low_voltage_limit(uint8_t *target_current)
{
    ...
    if (voltage_x100 <= lvc_ramp_down_start_voltage_x100)
    {
        ...
        uint8_t tmp = (uint8_t)MAP32(voltage_x100,                     // value
    ...
    if (active)
    {
        ...
        if (!shift_sensor_interrupting)
        {
            uint16_t duration_ms =
                EXPAND_U16(g_config.shift_interrupt_duration_ms_u16h, g_config.shift_interrupt_duration_ms_u16l);
...
void reload_assist_params()
{
    if (assist_level < ASSIST_PUSH)
    {
        ...
        assist_level_data.max_wheel_speed_rpm_x10 =
            ((int32_t)global_speed_limit_rpm * assist_level_data.level.max_speed_percent) / 10;

        if (assist_level_data.level.flags & ASSIST_FLAG_PAS)
        {
            assist_level_data.keep_current_target_percent =
                (uint8_t)((uint16_t)g_config.pas_keep_current_percent * assist_level_data.level.target_current_percent /
                          100);
            assist_level_data.keep_current_ramp_start_rpm_x10 = g_config.pas_keep_current_cadence_rpm * 10;
            assist_level_data.keep_current_ramp_end_rpm_x10 =
                (uint16_t)(((uint32_t)assist_level_data.level.max_cadence_percent * MAX_CADENCE_RPM_X10) / 100);
        }
...
uint16_t convert_wheel_speed_kph_to_rpm(uint8_t speed_kph)
{
    float radius_mm = EXPAND_U16(g_config.wheel_size_inch_x10_u16h, g_config.wheel_size_inch_x10_u16l) *
                      1.27f; // g_config.wheel_size_inch_x10 / 2.f * 2.54f;
